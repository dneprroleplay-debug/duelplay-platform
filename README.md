...
## MASTER CHECKLIST v30
The v29 source was extended in place. `DUELPLAY_MASTER_CHECKLIST_V30_STATUS.md` records the implementation boundary for all 68 checklist items.

For production, run `prisma generate` and create/apply a real Prisma migration for the changed schema on the target PostgreSQL database. Run `npm run match-watchdog` from a scheduler at least once per minute (or call the protected `/api/jobs/match-watchdog` endpoint). Real payment providers, Steam Trade, external creator integrations and production CS2 hosts still require their external credentials/infrastructure.


## Local MASTER CHECKLIST setup
1. `npm install`
2. Configure `.env` from `.env.example`.
3. `npm run db:validate`
4. `npm run db:sync` to synchronize an existing local v29 database with the v30 schema without deleting the database.
5. `npm run prisma:generate`
6. `npm run build`
7. `npm run dev`

The local database sync is intentionally explicit so production deployments never mutate the database just by starting the app.
