...
export function levelFromXp(xp:number){
  ...
  return Math.max(0,Math.min(100,Math.floor(Math.sqrt(safe/100))));
}
...
export async function awardXp(tx:Prisma.TransactionClient,userId:string,amount:number){
  ...
  const locked=await tx.$queryRaw<Array<{xp:number;level:number}>>`SELECT xp, level FROM "User" WHERE id = ${userId}::uuid FOR UPDATE`;
  const user=locked[0];
  if(!user)return null;
  const now=new Date();
...
export async function updateMatchProgress(tx:Prisma.TransactionClient,userId:string,winner:boolean){
  ...
  const stats=await tx.playerStats.upsert({where:{userId},update:{totalDuels:{increment:1},wins:winner?{increment:1}:undefined,losses:winner?undefined:{increment:1},winStreak:winner?{increment:1}:0},create:{userId,totalDuels:1,wins:winner?1:0,losses:winner?0:1,winStreak:winner?1:0,bestStreak:winner?1:0,rating:0}});
...
export async function updateRatingAfterDuel(tx:Prisma.TransactionClient,winnerId:string,loserId:string){
  const [winner,loser]=await Promise.all([
    tx.playerStats.upsert({where:{userId:winnerId},update:{},create:{userId:winnerId,rating:0}}),
    tx.playerStats.upsert({where:{userId:loserId},update:{},create:{userId:loserId,rating:0}}),
  ]);
...
export async function recordMatchStats(tx:Prisma.TransactionClient,matchId:string,userId:string,input:{kills?:unknown;assists?:unknown;deaths?:unknown;headshots?:unknown;damage?:unknown;score?:unknown;weapon?:unknown;mapName?:unknown}){
  ...
  const kills=n(input.kills);
  const assists=n(input.assists);
  const deaths=n(input.deaths);
  const headshots=n(input.headshots);
  const damage=n(input.damage);
  const score=n(input.score);
  const stat = await tx.matchPlayerStat.upsert({where:{matchId_userId:{matchId,userId}},update:{kills,assists,deaths,headshots,damage,score,weapon:typeof input.weapon==='string'?input.weapon.slice(0,80):undefined,mapName:typeof input.mapName==='string'?input.mapName.slice(0,80):undefined},create:{matchId,userId,kills,assists,deaths,headshots,damage,score,weapon:typeof input.weapon==='string'?input.weapon.slice(0,80):undefined,mapName:typeof input.mapName==='string'?input.mapName.slice(0,80):undefined}});
  await tx.playerStats.upsert({
    where:{userId},
    update:{kills:{increment:kills},deaths:{increment:deaths},headshots:{increment:headshots},damage:{increment:damage}},
    create:{userId,rating:0,kills,deaths,headshots,damage},
  });
  return stat;
}
