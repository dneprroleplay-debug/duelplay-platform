...
export type ThemeBackgroundId = typeof THEME_BACKGROUNDS[number];

export const THEME_HEROES = Array.from({length:10},(_,i)=>`hero-${String(i+1).padStart(2,"0")}`) as string[];
...
export const isThemeBackgroundId=(id:unknown): id is ThemeBackgroundId => typeof id === "string" && THEME_BACKGROUNDS.includes(id as ThemeBackgroundId);
export const isThemeHeroId=(id:unknown): boolean => typeof id === "string" && THEME_HEROES.includes(id);
