...
assert.match(claim,/status:\s*\{ in: \[\"OFFLINE\", \"ERROR\"\] \}, matchId:\s*null/,'server claim can recover an ERROR server with no assigned match');
assert.match(claim,/existing\.status !== \"OFFLINE\" && existing\.status !== \"ERROR\"/,'server claim still blocks genuinely busy non-OFFLINE servers');
...
assert.doesNotMatch(page,/loadPresence\(\)/,'match page does not maintain a second state-writing presence poller');
assert.doesNotMatch(page,/setPresence/,'match page does not keep a separate presence state that can race with match state');
assert.match(presence,/playerTwo:/,'presence endpoint still exposes the second player for server-side consumers');
...
assert.match(page,/const loadInFlight=useRef\(false\)/,'match polling prevents overlapping requests');
console.log('Presence/server deadline and match-state race regression checks passed.');
