...
assert.match(lifecycle, /maxWait: 10000, timeout: 15000/);
...
assert.match(lifecycle, /if \(ids\.length === 0\) await cancelMatchWithRefund/);
assert.match(lifecycle, /else if \(ids\.length === 1\) await resolveConnectionTimeout/);
