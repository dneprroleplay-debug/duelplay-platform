...
try {
  for (const account of accounts) {
    const user = await prisma.user.upsert({
      ...
      create: { ...account, role: 'USER', status: 'ACTIVE', isTestAccount: true, language: 'ru', wallet: { create: { balance: 500 } } },
      select: { id: true, nickname: true, isTestAccount: true },
    });
    await prisma.wallet.upsert({ where: { userId: user.id }, update: {}, create: { userId: user.id, balance: 500 } });
