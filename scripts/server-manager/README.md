...
`DUELPLAY_SERVER_MANAGER_SECRET` и `CS2_RESULT_SECRET` должны быть разными длинными случайными секретами. Первый используется для Server Manager API, второй — только для подтверждения результатов CS2.
