...
async function main() {
  ...
  await prisma.wallet.upsert({ where: { userId: test.id }, update: {}, create: { userId: test.id, balance: 500 } });
  ...
  await prisma.wallet.upsert({ where: { userId: rival.id }, update: {}, create: { userId: rival.id, balance: 500 } });
