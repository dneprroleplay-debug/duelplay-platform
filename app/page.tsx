            ...
            <div className="panel overflow-hidden rounded-3xl border-pink-400/15 bg-black/45 p-5 backdrop-blur">
              ...
              <div className="space-y-3 pt-4">
                {openMatches.length>0?openMatches.map((m:any)=><Link href={`/matches/${m.id}`} key={m.id} className="flex items-center gap-3 rounded-2xl border border-white/5 bg-white/[.025] p-3 transition hover:border-pink-400/25 hover:bg-pink-400/[.035]">
                  ...
                  <div className="min-w-0 flex-1"><div className="font-bold">{m.mapName||"Mirage"}</div><div className="mt-1 text-xs text-zinc-500">1/2 игроков · 1v1 · {m.playerOne?.nickname||"Игрок"}</div></div>
                  <div className="text-right"><div className="font-black text-pink-300">${Number(m.betAmount).toFixed(2)}</div><div className="text-[10px] text-zinc-600">СТАВКА</div></div>
