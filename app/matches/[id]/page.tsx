...
export default function MatchPage({params}:{params:Promise<{id:string}>}){
 ...
 async function loadMatch(){
   ...
   try{
     const mr=await fetch(`/api/matches/${id}?t=${Date.now()}-${Math.random().toString(36).slice(2)}`,{cache:"no-store",headers:{"Cache-Control":"no-store"},signal:controller.signal});
     ...
     // here can leave the user on a transient loading screen while the API resolves.
   }catch(error){if((error as Error)?.name!=="AbortError")setMsg(t.matchLoadError)}
   finally{window.clearTimeout(timeout);loadInFlight.current=false}
 }
 ...
 },[m?.status,m?.startDeadlineAt,m?.liveDeadlineAt,m?.startedAt]);
...
function openWorkshopMap(){
 const map=getDuelMap(m?.mapName);
 if(!map?.workshopId)return;
 const url=`https://steamcommunity.com/sharedfiles/filedetails/?id=${encodeURIComponent(map.workshopId)}`;
...
useEffect(()=>{
 if(m?.status==="FINISHED"||m?.status==="CANCELLED"){
   void refreshAuth();
   window.dispatchEvent(new Event("duelplay:wallet-updated"));
   window.dispatchEvent(new Event("duelplay:profile-updated"));
   window.dispatchEvent(new Event("duelplay:notifications-updated"));
 }
...
async function localFinish(winnerId:string){setBusy(true);setMsg("");try{const r=await fetch(`/api/matches/${id}/local-test`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({winnerId})});const d=await r.json();if(!r.ok)throw new Error(d.error||t.startError);await load();await refreshAuth();window.dispatchEvent(new Event("duelplay:profile-updated"));showMessage(t.finished)}catch(e){showMessage(e instanceof Error?e.message:t.startError)}finally{setBusy(false)}}
 ...
 const p1=displayPlayerOne?.avatarUrl||displayPlayerOne?.steamAvatarUrl||"/avatars/premium/01-cyan.svg",p2=displayPlayerTwo?.avatarUrl||displayPlayerTwo?.steamAvatarUrl||"/avatars/premium/02-violet.svg";
 return <><main className="match-page-shell mx-auto min-h-screen max-w-6xl px-4 pb-20 pt-24 sm:px-6">
  ...
  <section className="match-panel panel overflow-hidden rounded-[28px]">
   <div className="relative h-56 sm:h-72"><Image src={mapImage||"/images/maps/mirage.jpg"} alt={m.mapName||"CS2"} fill className="object-cover"/><div className="absolute inset-0 bg-[linear-gradient(90deg,rgba(5,5,7,.96),rgba(5,5,7,.35),rgba(5,5,7,.96))]"/><div className="absolute inset-0 bg-[radial-gradient(circle_at_50%_50%,rgba(255,47,145,.18),transparent_45%)]"/><div className="absolute inset-x-5 bottom-7 text-center"><div className="text-xs font-black tracking-[.25em] text-[var(--theme-accent)]">CS2 · 1х1 DUEL</div><h1 data-no-i18n className="mt-2 text-3xl font-black sm:text-5xl">Match #{m.id.slice(0,8).toUpperCase()}</h1><p className="mt-2 text-sm font-semibold text-white drop-shadow-[0_1px_3px_rgba(0,0,0,.85)]"><span data-no-i18n>{m.mapName||"Mirage"}</span> · 1х1 · {weaponLabel(m.weaponModifier) ? `${weaponLabel(m.weaponModifier)} · ` : ""}${Number(m.betAmount).toFixed(2)}</p></div></div>
   <div className="p-5 sm:p-8">
    ...
    <div className="mt-7 grid gap-4 md:grid-cols-[minmax(0,1fr)_auto_minmax(0,1fr)] md:items-stretch">
      <PlayerCard player={displayPlayerOne} avatar={p1} crown={true} bet={Number(m.betAmount)} ready={m.status!=="WAITING_FOR_PLAYERS"} u={u}/><div className="hidden items-center justify-center md:flex text-2xl font-black text-[var(--theme-accent)]">VS</div>
      <PlayerCard player={displayPlayerTwo} avatar={p2} bet={Number(m.betAmount)} ready={hasPlayerTwo} u={u}/>
    </div>
    ...
    {m.status==="READY"&&<div className="mt-5 rounded-2xl border border-amber-400/20 bg-amber-400/[.04] p-5"><div className="flex items-center justify-between gap-4"><div className="min-w-0"><div className="text-sm font-black text-amber-300">{u.ready}</div><div className="mt-1 text-sm text-zinc-500">{m.serverConfig?.managerRequested?u.serverStarting:u.bothReady}</div></div>{currentDeadline()&&<div className="shrink-0 text-right"><div className="text-[10px] font-black uppercase tracking-wider text-amber-400/60">{u.timeout}</div><DeadlineCountdown deadline={currentDeadline()!.at} maxSeconds={currentDeadline()!.maxSeconds} className="mt-1 font-mono text-2xl font-black tracking-wider text-amber-200" onExpire={()=>{setDeadlineExpired(true);void load()}}/></div>}</div></div>}
...
{currentDeadline()&&<div className="mt-5 rounded-2xl border border-amber-400/20 bg-amber-400/[.04] p-4">
  <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
    <div className="min-w-0 flex-1">
      <div className="text-[10px] font-black uppercase tracking-wider text-amber-400/70">
        LIVE
      </div>
      <p className="mt-2 text-xs leading-5 text-zinc-400">{u.connectionRule}</p>
    </div>

    <div className="shrink-0 text-right sm:min-w-[150px]">
      <div className="font-mono text-3xl font-black tracking-wider text-amber-200">
        <DeadlineCountdown deadline={currentDeadline()!.at} maxSeconds={currentDeadline()!.maxSeconds} onExpire={()=>{setDeadlineExpired(true);void load()}}/>
      </div>
...
{m.serverConfig?.localTest&&m.playerTwo&&<div className="mt-5 rounded-xl border border-amber-400/20 bg-amber-400/[.04] p-4">
  ...
  <div className="mt-3 flex flex-wrap gap-3">
    <button disabled={busy} onClick={()=>localFinish(m.playerOneId)} className="rounded-xl bg-[var(--theme-accent)] px-4 py-3 text-sm font-black text-black disabled:opacity-50">{u.testWin}: <span data-player-name>{m.playerOne.nickname}</span></button>
    <button disabled={busy} onClick={()=>displayPlayerTwo&&localFinish(displayPlayerTwo.id||"")} className="rounded-xl border border-white/10 bg-white/5 px-4 py-3 text-sm font-black disabled:opacity-50">{u.testWin}: <span data-player-name>{displayPlayerTwo?.nickname||u.waitingPlayer}</span></button>
  </div>
</div>}

{(mapConfig?.workshopId||m.serverConfig?.connectUrl)&&<div className="mt-5 flex flex-wrap gap-3">
  {mapConfig?.workshopId&&<button type="button" onClick={openWorkshopMap} className="inline-flex min-h-[48px] items-center justify-center rounded-xl border border-white/10 bg-white/[.03] px-5 py-3 text-sm font-black text-white transition hover:bg-white/[.07]">
    {u.downloadMap}
  </button>}
  {m.serverConfig?.connectUrl&&<button type="button" onClick={openServer} disabled={!m.serverConfig.connectUrl||busy} className="inline-flex min-h-[48px] min-w-[220px] items-center justify-center rounded-xl border border-emerald-400/25 bg-emerald-400/[.03] px-5 py-3 text-sm font-black text-emerald-300 transition hover:bg-emerald-400/10 disabled:opacity-60">
    ...
    {m.status==="FINISHED"&&<Notice title={u.confirmed}>{u.winner}: <b data-player-name>{m.winner?.nickname||"—"}</b> · {u.payout} <b>${payout.toFixed(2)}</b></Notice>}
    ...
    <div className="mt-6 flex flex-wrap gap-3">{participant&&m.status==="READY"&&!m.serverConfig?.managerRequested&&!deadlineExpired&&<button disabled={busy} onClick={()=>action("start")} className="rounded-2xl bg-[var(--theme-accent)] px-6 py-4 font-black text-black disabled:opacity-50">START</button>}{participant&&m.status==="WAITING_FOR_PLAYERS"&&user?.id===m.playerOneId&&<button disabled={busy} onClick={()=>setCancelOpen(true)} className="cancel-match rounded-2xl border border-red-400/20 px-5 py-3 font-bold text-red-300">{t.cancel}</button>}</div>
  ...
  <CenterModal open={cancelOpen} title={u.cancelMatch} className="cancel-match-modal" contentClassName="p-5 sm:p-6" onClose={()=>{if(!busy)setCancelOpen(false)}}>
...
function DeadlineCountdown({deadline,maxSeconds,className="",onExpire}:{deadline:string;maxSeconds:number;className?:string;onExpire?:()=>void}){
 ...
 useEffect(()=>{
   ...
   if(!Number.isFinite(deadlineMs)){if(ref.current)ref.current.textContent="--:--";return}
   let frame=0;
   let lastShown=-1;
   const paint=()=>{
     // Direct DOM painting is intentional: API polling can re-render the page,
     // but it cannot freeze/reset this clock. `floor` + clamp also prevents 10:01/02:01.
     const raw=Math.floor((deadlineMs-Date.now())/1000);
     const left=Math.max(0,Math.min(maxSeconds,raw));
     if(left!==lastShown){
       lastShown=left;
       if(ref.current)ref.current.textContent=formatCountdown(left);
     }
     if(left<=0){
 ...
 return <span ref={ref} className={className}>{formatCountdown(maxSeconds)}</span>;
}
function PlayerCard({player,avatar,crown,bet,ready,u}:{player:Player|null;avatar:string;crown?:boolean;bet:number;ready:boolean;u:any}){return <div className="rounded-2xl border border-white/7 bg-white/[.025] p-4 sm:p-5"><div className="flex items-center gap-4"><img src={avatar} alt="" className="h-16 w-16 rounded-2xl object-cover border border-white/10"/><div className="min-w-0 flex-1"><div className="flex items-center gap-2"><b data-player-name className="truncate">{player?.nickname||u.waitingPlayer}</b>{crown&&<span title={u.creator} className="text-lg">👑</span>}</div><div className="mt-1 text-xs text-zinc-500">{player?`${u.bet} $${bet.toFixed(2)}`:u.readyConnect}</div></div></div><div className={`mt-5 inline-flex rounded-full px-3 py-1 text-[10px] font-black tracking-wider ${ready?"bg-emerald-400/10 text-emerald-300":"bg-amber-400/10 text-amber-300"}`}>{ready?"READY":u.waitingStatus}</div></div>}
