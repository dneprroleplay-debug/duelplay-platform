...
export default function MatchPage({params}:{params:Promise<{id:string}>}){
 ...
 async function loadMatch(){if(!id)return;matchAbortRef.current?.abort();const controller=new AbortController();matchAbortRef.current=controller;const timeout=window.setTimeout(()=>controller.abort(),4000);try{const mr=await fetch(`/api/matches/${id}?t=${Date.now()}-${Math.random().toString(36).slice(2)}`,{cache:"no-store",headers:{"Cache-Control":"no-store"},signal:controller.signal});const md=await mr.json();if(!mr.ok)throw 0;const fresh=md.match??md;setM(prev=>prev?{...prev,...fresh,playerOne:fresh.playerOne??prev.playerOne,playerTwo:fresh.playerTwo??prev.playerTwo,liveState:fresh.liveState??prev.liveState}:fresh)}catch(error){if((error as Error)?.name!=="AbortError")setMsg(t.matchLoadError)}finally{window.clearTimeout(timeout);if(matchAbortRef.current===controller)matchAbortRef.current=null}}
 async function loadPresence(){if(!id)return;try{const r=await fetch(`/api/matches/${id}/presence?t=${Date.now()}-${Math.random().toString(36).slice(2)}`,{cache:"no-store",headers:{"Cache-Control":"no-store"}});const d=await r.json();if(!r.ok||!d?.ok)return;setPresence(d);setM(prev=>{if(!prev)return prev;return {...prev,status:d.status??prev.status,playerTwo:d.playerTwo??prev.playerTwo,playerOne:d.playerOne??prev.playerOne,connectionDeadlineAt:d.connectionDeadlineAt??prev.connectionDeadlineAt,liveState:d.liveState??prev.liveState,serverConfig:prev.serverConfig?{...prev.serverConfig,...(d.serverConfig??{})}:d.serverConfig??prev.serverConfig}})}catch{}}
 ...
 useEffect(()=>{if(!id)return;void loadMatch();void loadPresence();void loadAuth()},[id]);
 useEffect(()=>{if(!id)return;const tick=()=>{if(document.visibilityState!=="hidden"){void loadPresence();void loadMatch()}};const timer=setInterval(tick,1000);const onVisibility=()=>{if(document.visibilityState==="visible"){void loadPresence();void loadMatch()}};document.addEventListener("visibilitychange",onVisibility);return()=>{clearInterval(timer);document.removeEventListener("visibilitychange",onVisibility);matchAbortRef.current?.abort()}},[id]);
 const load=loadMatch;
 useEffect(()=>{
   setDeadlineExpired(false);
 },[m?.status,m?.startDeadlineAt,m?.connectionDeadlineAt,m?.startedAt]);
 ...
 function currentDeadline(){
   ...
   if(m.status==="LIVE"&&m.liveState?.connectionPhaseCompleted!==true){
     if(m.connectionDeadlineAt)return {at:m.connectionDeadlineAt,maxSeconds:600};
     if(m.startedAt)return {at:new Date(new Date(m.startedAt).getTime()+10*60*1000).toISOString(),maxSeconds:600};
   }
   return null;
 }
...
async function localFinish(winnerId:string){setBusy(true);setMsg("");try{const r=await fetch(`/api/matches/${id}/local-test`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({winnerId})});const d=await r.json();if(!r.ok)throw new Error(d.error||t.startError);await load();await refreshAuth();showMessage(t.finished)}catch(e){showMessage(e instanceof Error?e.message:t.startError)}finally{setBusy(false)}}
 ...
 const displayPlayerOne=m.playerOne||presence?.playerOne;const displayPlayerTwo=m.playerTwo||presence?.playerTwo;const hasPlayerTwo=Boolean(displayPlayerTwo);const pot=Number(m.betAmount)*(hasPlayerTwo?2:1),payout=pot-Number(m.commission??pot*.10),participant=Boolean(user&&[m.playerOneId,m.playerTwoId].includes(user.id));const mapImage=MAP_IMAGES[m.mapName||""];
 ...
 const weaponLabel=(value:string|null|undefined)=>({AWP_ONLY:"AWP Only",DEAGLE_ONLY:"Deagle Only",KNIFE_ONLY:"Knife Only",HEADSHOT_ONLY:"Headshot Only",RANDOM_WEAPON:"Random Weapon",FIRST_TO_10:"First to 10",HIGH_STAKES:"High Stakes"}[String(value||"")]||"");
 ...
 return <><main className="match-page-shell mx-auto min-h-screen max-w-6xl px-4 pb-20 pt-24 sm:px-6">
  ...
  <section className="match-panel panel overflow-hidden rounded-[28px]">
   <div className="relative h-56 sm:h-72"><Image src={mapImage||"/images/maps/mirage.jpg"} alt={m.mapName||"CS2"} fill className="object-cover"/><div className="absolute inset-0 bg-[linear-gradient(90deg,rgba(5,5,7,.96),rgba(5,5,7,.35),rgba(5,5,7,.96))]"/><div className="absolute inset-0 bg-[radial-gradient(circle_at_50%_50%,rgba(255,47,145,.18),transparent_45%)]"/><div className="absolute inset-x-5 bottom-7 text-center"><div className="text-xs font-black tracking-[.25em] text-[var(--theme-accent)]">CS2 · 1х1 DUEL</div><h1 className="mt-2 text-3xl font-black sm:text-5xl">{u.matchTitle} #{m.id.slice(0,8).toUpperCase()}</h1><p className="mt-2 text-sm font-semibold text-white drop-shadow-[0_1px_3px_rgba(0,0,0,.85)]">{m.mapName||"Mirage"} · 1х1 · {weaponLabel(m.weaponModifier) ? `${weaponLabel(m.weaponModifier)} · ` : ""}${Number(m.betAmount).toFixed(2)}</p></div></div>
  ...
  <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
    ...
    <div className="shrink-0 text-right sm:min-w-[150px]">
      ...
      <div className="mt-1 text-[11px] text-zinc-500">
        {u.connected || "Connected"} {presence?.liveState?.connectedCount ?? m.liveState?.connectedCount ?? 0} / {presence?.liveState?.connectionSlots ?? m.liveState?.connectionSlots ?? 2}
      </div>
    </div>
  </div>
</div>}
...
{m.serverConfig?.localTest&&m.playerTwo&&<div className="mt-5 rounded-xl border border-amber-400/20 bg-amber-400/[.04] p-4">
  ...
  <div className="mt-3 flex flex-wrap gap-3">
    ...
    <button disabled={busy} onClick={()=>displayPlayerTwo&&localFinish(displayPlayerTwo.id||"")} className="rounded-xl border border-white/10 bg-white/5 px-4 py-3 text-sm font-black disabled:opacity-50">{u.testWin}: {displayPlayerTwo?.nickname||u.waitingPlayer}</button>
  </div>
</div>}
