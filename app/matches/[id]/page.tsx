  ...
  <section className="panel overflow-hidden rounded-[28px]">
   ...
   <div className="p-5 sm:p-8">
    ...
    {m.status==="WAITING_FOR_PLAYERS"&&<>
      <div className="mt-5 rounded-2xl border border-amber-400/20 bg-amber-400/[.04] p-5"><div className="flex items-center justify-between gap-4"><div className="min-w-0"><div className="flex items-center gap-2 text-sm font-black text-amber-300">{u.waiting}</div><p className="mt-2 text-sm leading-6 text-zinc-400">{u.waitingText}</p></div>{secondsLeft!==null&&<div className="mt-5 rounded-2xl border border-amber-400/20 bg-amber-400/[.04] p-4">
  <div className="flex flex-wrap items-center justify-between gap-4">
    <div>
      <div className="text-[10px] font-black uppercase tracking-wider text-amber-400/70">{u.connectTimeTitle||u.timeout}</div>
      <div className="mt-1 text-sm font-bold text-amber-300">{u.connectTimeText||u.connect}</div>
    </div>
    <div className="text-right">
      <div className="font-mono text-3xl font-black tracking-wider text-amber-200">{formatCountdown(secondsLeft)}</div>
      <div className="mt-1 text-[11px] text-zinc-500">{u.connected} {(m.serverConfig?.connectedSteamIds??[]).length} / 2</div>
    </div>
  </div>
</div>}
