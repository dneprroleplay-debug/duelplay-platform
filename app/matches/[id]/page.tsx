...
export default function MatchPage({params}:{params:Promise<{id:string}>}){
 ...
 async function loadMatch(){if(!id||loadInFlight.current)return;loadInFlight.current=true;const controller=new AbortController();const timeout=window.setTimeout(()=>controller.abort(),5000);try{const mr=await fetch(`/api/matches/${id}?t=${Date.now()}`,{cache:"no-store",signal:controller.signal});const md=await mr.json();if(!mr.ok)throw 0;setM(md.match??md)}catch(error){if((error as Error)?.name!=="AbortError")setMsg(t.matchLoadError)}finally{window.clearTimeout(timeout);loadInFlight.current=false}}
 async function loadAuth(){try{const ur=await fetch(`/api/auth/me?t=${Date.now()}`,{cache:"no-store"});const ud=await ur.json();setUser(ud.user??null)}catch{}}
 useEffect(()=>{if(!id)return;void loadMatch();void loadAuth()},[id]);
 useEffect(()=>{if(!id)return;const tick=()=>{if(document.visibilityState!=="hidden")void loadMatch()};const timer=setInterval(tick,2000);const onVisibility=()=>{if(document.visibilityState==="visible")void loadMatch()};document.addEventListener("visibilitychange",onVisibility);return()=>{clearInterval(timer);document.removeEventListener("visibilitychange",onVisibility)}},[id]);
