...
type User={id:string;nickname:string;email:string|null;avatarUrl:string|null;themePreference:string;level:number;xp:number;reputation:number;trustScore:string;referralCode:string;balance:string;lockedBalance:string};
...
export default function ProfilePage(){
 const {t}=useLanguage(); const {theme,setTheme}=useTheme(); const [data,setData]=useState<Data|null>(null); const [error,setError]=useState(""); const [copied,setCopied]=useState(false);
 useEffect(()=>{fetch("/api/profile",{cache:"no-store"}).then(async r=>{const d=await r.json();if(!r.ok){location.href="/login";return}setData(d)}).catch(()=>setError(t.profileLoadError))},[t.profileLoadError]);
 ...
 return <><Header/><main className="mx-auto min-h-screen max-w-6xl px-4 pb-20 pt-28">
  <div className="panel rounded-3xl p-7 sm:p-10">
   <div className="flex flex-col gap-7 lg:flex-row lg:items-start lg:justify-between">
    <div className="flex items-center gap-5"><AvatarPicker value={u.avatarUrl} nickname={u.nickname} onSaved={avatarUrl=>setData({...data,user:{...u,avatarUrl}})}/><div><span className="pill">{t.playerProfile}</span><h1 className="mt-3 text-4xl font-black">{u.nickname}</h1><p className="mt-2 text-zinc-500">{u.email}</p><p className="mt-2 text-xs text-zinc-600">{t.avatarHintShort}</p></div></div>
