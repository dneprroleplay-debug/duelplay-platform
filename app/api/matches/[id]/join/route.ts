...
export async function POST(_: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (!user) return NextResponse.json({ error: "Войдите, чтобы присоединиться" }, { status: 401 });
    ...
    if (msg === "NOT_FOUND") return NextResponse.json({ error: "Матч не найден" }, { status: 404 });
    if (msg === "OWN_MATCH") return NextResponse.json({ error: "Нельзя присоединиться к своему матчу" }, { status: 409 });
    if (msg === "FULL") return NextResponse.json({ error: "Матч уже заполнен" }, { status: 409 });
    if (msg === "INSUFFICIENT_BALANCE") return NextResponse.json({ error: "Недостаточно средств для входа в матч" }, { status: 400 });
    console.error(error); return NextResponse.json({ error: "Не удалось присоединиться" }, { status: 500 });
  }
}
