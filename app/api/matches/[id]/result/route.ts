...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const result = await prisma.$transaction(async tx => {
      ...
      await tx.notification.create({ data: { userId: winnerId, type: "TRANSACTION_SUCCESS", title: "Победа в дуэли", body: `Ты победил в CS2 1х1 и получил $${payout.toFixed(2)}.`, payload: { matchId: match.id, payout } } });
