...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const result = await prisma.$transaction(async tx => {
      ...
      await tx.transaction.create({ data: { walletId: winnerWallet.id, type: "MATCH_WIN", status: "COMPLETED", amount: payout, balanceBefore: wb, balanceAfter: wb + payout, referenceId: match.id, description: `CS2 referee result · $${payout.toFixed(2)}` } });
      ...
      await tx.notification.create({ data: { userId: winnerId, type: "TRANSACTION_SUCCESS", title: "Победа в дуэли", body: `Ты победил в CS2 1v1 и получил $${payout.toFixed(2)}.`, payload: { matchId: match.id, payout } } });
      await tx.notification.create({ data: { userId: loserId, type: "TRANSACTION_SUCCESS", title: "Матч завершён", body: "Дуэль завершена. Победитель подтверждён игровым сервером.", payload: { matchId: match.id } } });

      const serverCfg = record(match.serverConfig);
      const updated = await tx.match.update({ where: { id }, data: { winnerId, loserId, status: "FINISHED", endedAt: new Date(), serverConfig: { ...serverCfg, state: "FINISHED", resultSource: "CS2_SERVER" } } });
      return updated;
    });
