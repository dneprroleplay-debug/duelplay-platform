...
const DEFAULT_CASES: Array<{ slug: string; name: string; description: string; price: number; imageUrl: string; items: Array<[string, string, string, number, number]> }> = [
  { slug: "starter", name: "Starter Case", description: "Быстрый шанс получить первый предмет DuelPlay.", price: 1, imageUrl: "/images/cases/starter_case.png", items: [
    ["Pistol Core", "/images/case-items/pistol_core.png", "Common", 0.50, 72], ["Emerald Fang", "/images/case-items/emerald_fang.png", "Uncommon", 1.20, 20],
    ["Azure Strike", "/images/case-items/azure_strike.png", "Rare", 2.50, 6], ["Violet Pulse", "/images/case-items/violet_pulse.png", "Epic", 5, 1], ["Gold Reaper", "/images/case-items/gold_reaper.png", "Legendary", 12, 1]
  ] },
  { slug: "neon", name: "Neon Duel Case", description: "Неоновый кейс с редкими наградами.", price: 3, imageUrl: "/images/cases/neon_duel_case.png", items: [
    ["Emerald Fang", "/images/case-items/emerald_fang.png", "Uncommon", 1.50, 68], ["Azure Strike", "/images/case-items/azure_strike.png", "Rare", 3.50, 22],
    ["Violet Pulse", "/images/case-items/violet_pulse.png", "Epic", 7, 7], ["Gold Reaper", "/images/case-items/gold_reaper.png", "Legendary", 15, 2], ["Neon Wolf", "/images/case-items/neon_wolf.png", "Mythic", 30, 1]
  ] },
  { slug: "premium", name: "Premium Arsenal", description: "Премиальный кейс для охотников за редкими предметами.", price: 10, imageUrl: "/images/cases/premium_arsenal.png", items: [
    ["Azure Strike", "/images/case-items/azure_strike.png", "Rare", 5, 65], ["Violet Pulse", "/images/case-items/violet_pulse.png", "Epic", 12, 25],
    ["Gold Reaper", "/images/case-items/gold_reaper.png", "Legendary", 28, 9], ["Neon Wolf", "/images/case-items/neon_wolf.png", "Mythic", 80, 1]
  ] }
];
...
function resultPayload(item: { id: string; name: string; imageUrl: string; rarity: string; value: unknown }) {
  return { id: item.id, name: item.name, imageUrl: caseItemImage(item.name, item.imageUrl), rarity: item.rarity, value: Number(item.value) };
}
...
export async function GET(request: NextRequest) {
  try {
    ...
    return NextResponse.json(cases.map((c) => ({
      ...c,
      imageUrl: caseImage(c.slug, c.imageUrl),
      price: Number(c.price),
      items: c.items.map((i) => ({
        ...i,
        imageUrl: caseItemImage(i.name, i.imageUrl),
        value: Number(i.value),
        rarityChance: rarityChancePercent(i.rarity, c.items.map((x) => ({ rarity: x.rarity, weight: x.weight }))),
      })),
    })));
  } catch (e) {
    console.error(e);
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      const inventory = await tx.inventoryItem.create({ data: { userId: user.id, caseId: currentCase.id, caseItemId: won.id, name: won.name, imageUrl: caseItemImage(won.name, won.imageUrl), rarity: normalizeCaseRarity(won.rarity), value: won.value, status: "AVAILABLE" } });
