...
export async function PATCH(request: Request) {
  ...
  const updated = await prisma.user.update({ where: { id: user.id }, data: { avatarUrl }, select: { avatarUrl: true } });
  return NextResponse.json({ user: updated });
}
