...
import { DUEL_MAPS } from "@/lib/duel-maps";
...
export default function Games({onSelectMap}:{onSelectMap?:(name:string)=>void}) {
  ...
  return (
    <section id="maps" className="season-map-section relative mx-auto max-w-7xl overflow-hidden rounded-[2rem] px-4 py-20 sm:px-6">
      <div className="mb-8">
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        {DUEL_MAPS.map((map) => (
          <button
            onClick={() => select(map.serverMapName)}
            key={map.id}
          ...
          >
            <Image src={map.image} alt={map.displayName} fill sizes="(max-width: 640px) 100vw, (max-width: 1024px) 50vw, 25vw" className="object-cover transition duration-500 group-hover:scale-[1.045]" />
            <div className="absolute inset-0 bg-gradient-to-t from-black/85 via-black/10 to-transparent" />
            <div className="absolute inset-x-0 bottom-0 p-5">
              <div className="text-[10px] uppercase tracking-[.18em] text-pink-300">Counter-Strike 2</div>
              <div data-no-i18n className="mt-1 text-2xl font-black">{map.displayName}</div>
            </div>
          </button>
        ))}
      </div>
