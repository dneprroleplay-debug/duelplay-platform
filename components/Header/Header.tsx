...
import PerformanceModeControl from "../Common/PerformanceModeControl";
 ...
 const noticeBody=(n:Notice)=>{
  ...
  if(p.localTest===true){
   if(Number(p.payout||0)>0){
    ...
    return t.testMatchCredited.replace("{amount}", amount);
   }
   return t.localTestWinner;
  }
  ...
  if(n.type==="CANCELLATION"){
   ...
   if(language==="RU") return noConnect ? "Никто не подключился к серверу CS2 за 5 минут. Ставка возвращена." : serverTimeout ? "Сервер CS2 не запустился вовремя. Ставка возвращена." : "Матч автоматически отменён. Ставка возвращена.";
   if(language==="UA") return noConnect ? "Ніхто не підключився до сервера CS2 за 5 хвилин. Ставку повернено." : serverTimeout ? "Сервер CS2 не запустився вчасно. Ставку повернено." : "Матч автоматично скасовано. Ставку повернено.";
   if(language==="EN") return noConnect ? "Nobody connected to the CS2 server within 5 minutes. Your stake was refunded." : serverTimeout ? "The CS2 server did not start in time. Your stake was refunded." : "The match was automatically cancelled. Your stake was refunded.";
   return noConnect ? "Nikt nie połączył się z serwerem CS2 w ciągu 5 minut. Stawka została zwrócona." : serverTimeout ? "Serwer CS2 nie uruchomił się na czas. Stawka została zwrócona." : "Mecz został automatycznie anulowany. Stawka została zwrócona.";
  }
  const bodies:any={
    RU:{
...
>>>>>>> b4b250a (fix navigation and page flicker)
 ...
 return <>
  ...
  <header className="fixed inset-x-0 top-0 z-50 border-b border-white/10 bg-[#050507]/90 backdrop-blur-xl">
   <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-4 sm:px-6">
    ...
    <div className="flex items-center gap-2">
     ...
     {loading?<div className="h-10 w-28 animate-pulse rounded-xl border border-white/5 bg-white/[.03]"/>:user?<>
      ...
      <div className="relative" ref={profileRef}><button type="button" onClick={()=>{setProfileOpen(v=>!v);setLangOpen(false);setNoticeOpen(false)}} className="flex items-center gap-2 rounded-xl border border-white/10 bg-white/5 px-2.5 py-1.5 text-sm font-semibold transition hover:border-[var(--theme-accent)]/30">{avatar?<img src={avatar} alt="" className="h-8 w-8 rounded-lg object-cover"/>:<span className="grid h-8 w-8 place-items-center rounded-lg bg-white/5 text-xs font-black text-zinc-300">{String(user.nickname||"?").slice(0,1).toUpperCase()}</span>}<span data-player-name className="hidden max-w-28 truncate sm:inline">{user.nickname}</span><span className="text-xs text-zinc-500">⌄</span></button>{profileOpen&&<div className="absolute right-0 top-12 z-[60] w-64 max-w-[calc(100vw-1rem)] rounded-2xl border border-white/10 bg-[#0b0b10] p-2 shadow-2xl"><div className="border-b border-white/5 px-3 py-3"><div data-player-name className="font-bold">{user.nickname}</div><div data-no-i18n className="mt-1 text-xs text-zinc-500">Steam ID: {user.steamId||"—"}</div><div className="mt-2 text-lg font-black text-[var(--theme-accent)]">${Number(user.balance).toFixed(2)}</div></div><Link href="/profile" onClick={closeMenus} className="block rounded-xl px-3 py-2 text-sm hover:bg-white/5">{ui.profile}</Link><Link href="/hub" onClick={closeMenus} className="block rounded-xl px-3 py-2 text-sm text-[var(--theme-accent)] hover:bg-white/5">DuelPlay Hub</Link><Link href="/wallet" onClick={closeMenus} className="block rounded-xl px-3 py-2 text-sm hover:bg-white/5">{ui.wallet}</Link><Link href="/inventory" onClick={closeMenus} className="block rounded-xl px-3 py-2 text-sm hover:bg-white/5">{ui.inventory}</Link><Link href="/profile#theme" onClick={closeMenus} className="block rounded-xl px-3 py-2 text-sm hover:bg-white/5">{ui.appearance}</Link>{user.role&&user.role!=="USER"&&<Link href="/admin" onClick={closeMenus} className="block rounded-xl px-3 py-2 text-sm text-[var(--theme-accent)] hover:bg-white/5">{ui.admin}</Link>}<button onClick={()=>{closeMenus();logout()}} className="mt-1 w-full rounded-xl border border-red-400/20 px-3 py-2 text-left text-sm text-red-300 hover:bg-red-500/10">{ui.logout}</button></div>}</div>
   ...
   {menuOpen&&<div onClick={e=>e.stopPropagation()} className="border-t border-white/10 bg-[#050507] px-4 py-3 xl:hidden"><nav className="flex flex-col gap-1">{links}</nav>{user&&<div className="mt-2 grid grid-cols-2 gap-2 border-t border-white/5 pt-3 sm:grid-cols-4"><Link href="/search" onClick={closeMenus} className="rounded-xl border border-white/10 px-3 py-2 text-center text-sm">{t.search}</Link><Link href="/create" onClick={closeMenus} className="rounded-xl bg-[var(--theme-accent)] px-3 py-2 text-center text-sm font-black text-black">{t.create}</Link><Link href="/wallet" onClick={closeMenus} className="rounded-xl border border-white/10 px-3 py-2 text-center text-sm">{ui.wallet}</Link><Link href="/inventory" onClick={closeMenus} className="rounded-xl border border-white/10 px-3 py-2 text-center text-sm">{ui.inventory}</Link></div>}</div>}
  </header>
 </>;
}
