...
export default function Live({refreshKey=0,mode="all",showFilters=false}:{refreshKey?:number;mode?:LobbyMode;showFilters?:boolean}){
  ...
  async function load(){try{setError("");const query=mode==="waiting"?"?status=waiting":mode==="active"?"?status=active":"";const data=await getMatches(query);setMatches(Array.isArray(data)?data:[])}catch(e){const code=e instanceof Error?e.message:"";setError(code==="MATCHES_LOAD_ERROR"?t.genericLoadError:code==="AUTH_REQUIRED"?t.loginRequired:t.genericLoadError)}finally{setLoading(false)}}
  useEffect(()=>{
