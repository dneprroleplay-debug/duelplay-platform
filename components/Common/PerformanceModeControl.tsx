...
export default function PerformanceModeControl(){
 ...
 return <div ref={ref} className="relative">
  <button type="button" aria-label="Performance mode" title="Performance mode" onClick={()=>setOpen(v=>!v)} className="grid h-10 w-10 place-items-center rounded-xl border border-white/10 bg-white/5 text-sm hover:border-[var(--theme-accent)]/40 hover:text-[var(--theme-accent)]">⚡</button>
  {open&&<div className="absolute right-0 top-12 z-[70] w-[300px] max-w-[calc(100vw-1rem)] rounded-2xl border border-white/10 bg-[#0b0b10]/95 p-4 shadow-2xl backdrop-blur-xl">
   <div className="flex items-center justify-between gap-3"><div><b className="text-sm">Performance</b><div className="mt-1 text-[11px] text-zinc-500">For your screen only</div></div><span className="text-[10px] font-black uppercase tracking-widest text-[var(--theme-accent)]">{effective}</span></div>
   ...
   <p className="mt-3 text-[11px] leading-5 text-zinc-500">AUTO selects a mode based on your device and reduced-motion setting. PERFORMANCE reduces decorative particles and disables heavy seasonal effects.</p>
  </div>}
 </div>;
}
