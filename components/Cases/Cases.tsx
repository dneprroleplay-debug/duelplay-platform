...
import { caseImage, caseItemImage } from "@/lib/case-assets";
...
const CASE_COPY:any={
 ...
 RU:{starter:["Starter Case","A quick chance to get your first DuelPlay item."],neon:["Neon Duel Case","A neon case with rare rewards."],premium:["Premium Arsenal","A premium case for hunters of rare items."]},
 UA:{starter:["Starter Case","A quick chance to get your first DuelPlay item."],neon:["Neon Duel Case","A neon case with rare rewards."],premium:["Premium Arsenal","A premium case for hunters of rare items."]},
 PL:{starter:["Starter Case","A quick chance to get your first DuelPlay item."],neon:["Neon Duel Case","A neon case with rare rewards."],premium:["Premium Arsenal","A premium case for hunters of rare items."]}
};
...
export default function Cases(){
 ...
 const[cases,setCases]=useState<DuelCase[]>([]),[selected,setSelected]=useState<DuelCase|null>(null),[opening,setOpening]=useState(false),[spinning,setSpinning]=useState(false),[won,setWon]=useState<CaseItem|null>(null),[roll,setRoll]=useState<RollItem[]>([]),[error,setError]=useState(""),[rollTarget,setRollTarget]=useState<number|null>(null),[stopRatio,setStopRatio]=useState(.5),[recovering,setRecovering]=useState(false);
 const viewportRef=useRef<HTMLDivElement>(null); const trackRef=useRef<HTMLDivElement>(null); const openingKeyRef=useRef<string|null>(null);
 const pendingStorageKey="duelplay:case-opening:v1";
 useEffect(()=>{
   let cancelled=false;
   fetch("/api/cases",{cache:"no-store"}).then(r=>r.json()).then(async d=>{
     if(cancelled||!Array.isArray(d))return;
     setCases(d.map((box:any)=>({...box,imageUrl:caseImage(box.slug,box.imageUrl),items:Array.isArray(box.items)?box.items.map((item:any)=>({...item,imageUrl:caseItemImage(item.name,item.imageUrl)})):[]})));
     try{
       const raw=sessionStorage.getItem(pendingStorageKey); if(!raw)return;
       const pending=JSON.parse(raw) as {slug?:string;key?:string};
       if(!pending.slug||!pending.key)return;
       const rawBox=d.find((item:DuelCase)=>item.slug===pending.slug); if(!rawBox)return;
       const box:{id:string;slug:string;name:string;description:string;price:number;imageUrl:string;items:CaseItem[]}={...rawBox,imageUrl:caseImage(rawBox.slug,rawBox.imageUrl),items:Array.isArray(rawBox.items)?rawBox.items.map((item:CaseItem)=>({...item,imageUrl:caseItemImage(item.name,item.imageUrl)})):[]};
       const check=await fetch(`/api/cases?openingKey=${encodeURIComponent(pending.key)}`,{cache:"no-store"});
       const result=await check.json(); if(cancelled||!check.ok||!result?.found)return;
       openingKeyRef.current=pending.key; setSelected(box); setWon(result.item as CaseItem); setSpinning(false); setOpening(false); setRecovering(true); sessionStorage.removeItem(pendingStorageKey);
     }catch{}
   }).catch(()=>{});
 ...
 const close=()=>{if(!opening){setSelected(null);setWon(null);setRoll([]);setRollTarget(null);setSpinning(false);setError("");setRecovering(false);openingKeyRef.current=null; try{sessionStorage.removeItem(pendingStorageKey)}catch{}}};
 function getOpeningKey(box:DuelCase){
   if(openingKeyRef.current)return openingKeyRef.current;
   try{
     const raw=sessionStorage.getItem(pendingStorageKey); if(raw){const pending=JSON.parse(raw) as {slug?:string;key?:string}; if(pending.slug===box.slug&&pending.key){openingKeyRef.current=pending.key;return pending.key}}
   }catch{}
   ...
   try{sessionStorage.setItem(pendingStorageKey,JSON.stringify({slug:box.slug,key}))}catch{}
   return key;
 }
 async function openCase(box:DuelCase){
   if(opening)return;
   const target=chooseTarget(); const ratio=chooseStopRatio(); const key=getOpeningKey(box);
   setOpening(true);setSpinning(false);setRecovering(false);setError("");setWon(null);
   setRollTarget(target);setStopRatio(ratio);
   if(!roll.length)setRoll(makeSequence(box,box.items[0],target));
   try{
    const r=await fetch("/api/cases",{method:"POST",headers:{"Content-Type":"application/json","idempotency-key":key},body:JSON.stringify({slug:box.slug})});
    ...
    setWon(winner); setRecovering(false); try{sessionStorage.removeItem(pendingStorageKey)}catch{}; openingKeyRef.current=null;
 ...
 <CenterModal open={Boolean(selected)} title={selected?copy(selected.slug,selected)[0]:t.case} onClose={close} fullscreen className="bg-[#050507]" contentClassName="h-[calc(100dvh-73px)] overflow-auto p-0">
   {selected&&<div className="min-h-full pb-10">{won?<WinCard won={won} onClose={close} t={t} rarity={rarity}/>:<>
    ...
    {!opening&&<button onClick={()=>void openCase(selected)} className="mx-auto mt-7 block w-full max-w-5xl rounded-2xl bg-[var(--theme-accent)] py-4 text-lg font-black text-black shadow-[0_0_45px_var(--theme-glow)]">{error?"Retry opening":t.caseOpen+" $"+selected.price.toFixed(2)}</button>}
    {recovering&&<div className="mx-auto mt-3 max-w-5xl text-center text-xs text-zinc-500">Recovered the confirmed result of the previous opening.</div>}
    </div></>}</div>}
 </CenterModal></section>
}
function chance(item:CaseItem,items:CaseItem[]){const valid=items.filter(x=>Number.isSafeInteger(x.weight)&&x.weight>0);const total=valid.reduce((n,x)=>n+x.weight,0);return total&&item.weight>0?((item.weight/total)*100).toFixed(1):"0.0"}
function CaseCard({box,copy,rarity,openText,onOpen,t}:{box:DuelCase;copy:string[];rarity:(v:string)=>string;openText:string;onOpen:()=>void;t:any}){return <article className="group overflow-hidden rounded-3xl border border-white/8 bg-white/[.025] transition hover:-translate-y-1 hover:border-[var(--theme-accent)]/35"><div className="relative h-48 overflow-hidden bg-black"><img src={caseImage(box.slug,box.imageUrl)} alt="" className="h-full w-full object-cover transition duration-500 group-hover:scale-105"/><div className="absolute inset-0 bg-gradient-to-t from-black via-black/30 to-transparent"/><div className="absolute left-4 top-4 rounded-full border border-[var(--theme-accent)]/30 bg-black/60 px-3 py-1 text-[10px] font-black tracking-widest text-[var(--theme-accent)]">{t.caseBadge}</div><div className="absolute bottom-4 left-4 right-4"><div className="text-xl font-black">{copy[0]}</div><div className="mt-1 text-xs text-zinc-300">{copy[1]}</div></div></div><div className="p-4"><div className="mb-4 grid grid-cols-2 gap-2">{box.items.slice(0,4).map(item=><div key={item.id} className="rounded-xl border border-white/5 bg-black/20 px-2.5 py-2"><div className="flex items-center gap-2"><img src={caseItemImage(item.name,item.imageUrl)} alt="" className="h-9 w-9 shrink-0 rounded-lg object-cover"/><div className="min-w-0 flex-1"><div className="truncate text-[11px] font-bold">{item.name}</div><div className="mt-1 flex justify-between text-[10px] text-zinc-600"><span>{rarity(item.rarity)}</span><span>{chance(item,box.items)}%</span></div></div></div></div>)}</div><button onClick={onOpen} className="w-full rounded-xl bg-[var(--theme-accent)] py-3 font-black text-black">{openText} ${box.price.toFixed(2)}</button></div></article>}
function WinCard({won,onClose,t,rarity}:{won:CaseItem;onClose:()=>void;t:any;rarity:(v:string)=>string}){return <div className="mx-auto max-w-6xl px-5 py-10 sm:px-10"><div className="text-center"><div className="text-sm font-black uppercase tracking-[.3em] text-[var(--theme-accent)]">{t.congratulations}</div><h3 className="mt-3 text-4xl font-black sm:text-6xl">{t.youWon}</h3><div className="mt-2 text-2xl font-black text-[var(--theme-accent)] sm:text-4xl">{won.name}</div><div className="mt-2 text-3xl font-black sm:text-5xl">${won.value.toFixed(2)}</div></div><div className="mx-auto mt-8 max-w-3xl overflow-hidden rounded-3xl border border-[var(--theme-accent)]/35 bg-[var(--theme-accent-bg)] p-5 shadow-[0_0_70px_var(--theme-glow)]"><img src={caseItemImage(won.name,won.imageUrl)} alt="" className="mx-auto aspect-[16/7] w-full rounded-2xl object-cover"/><div className="mt-3 text-center text-xs uppercase tracking-widest text-zinc-500">{rarity(won.rarity)}</div></div><p className="mt-5 text-center text-sm text-zinc-500">{t.itemStored}</p><div className="mt-7 flex flex-wrap justify-center gap-3"><Link href="/inventory" onClick={onClose} className="rounded-xl bg-[var(--theme-accent)] px-6 py-3 font-black text-black">{t.openInventory}</Link><button onClick={onClose} className="rounded-xl border border-white/10 px-6 py-3 font-bold">{t.close}</button></div></div>}
