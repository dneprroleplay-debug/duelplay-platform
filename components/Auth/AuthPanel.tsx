  ...
  return <div className="mx-auto max-w-md pt-10">
    <div className="panel rounded-3xl p-7 sm:p-9">
      ...
      <div className="mt-6 rounded-2xl border border-white/10 bg-black/20 p-4">
        <label className="text-xs font-bold uppercase tracking-wider text-zinc-400">{t.referralCodeOptional}</label>
        <input value={referralCode} onChange={e=>setReferralCode(e.target.value.toUpperCase().replace(/[^A-Z0-9_-]/g,""))} placeholder="DPXXXXXXXX" className="input mt-2 w-full" autoComplete="off" />
        <p className="mt-2 text-xs leading-5 text-zinc-600">{t.referralRegisterHint}</p>
      </div>
